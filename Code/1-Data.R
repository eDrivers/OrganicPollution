...
fileID <- data.frame(id     = 'plumesPesticides',
                     year   = 2013,
                     link   = "https://knb.ecoinformatics.org/knb/d1/mn/v2/object/raw_2013_plumes_pest_mol_20150714093921",
                     export = "raw_2013_plumes_pest_mol.zip",
                     stringsAsFactors = F)
...
org <- rasterToPolygons(org)

# Transform to sf object
org <- st_as_sf(org)

# Select only features with values > 0
id0 <- org$plumes_pest > 0
org <- org[id0, ]
...
save(org, file = './data/rawData/organic.RData')
